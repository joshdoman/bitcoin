...
const std::map<std::string, unsigned int> FLAG_NAMES = {
    ...
    {std::string("GRAFTLEAF"), (unsigned int)SCRIPT_VERIFY_GRAFTLEAF},
};
...
std::vector<unsigned int> AllFlags()
{
    ...
    for (unsigned int i = 0; i < 256; ++i) {
        ...
        if (i & 128) flag |= SCRIPT_VERIFY_GRAFTLEAF;
        ...
        // SCRIPT_VERIFY_GRAFTLEAF requires SCRIPT_VERIFY_TAPROOT
        if (flag & SCRIPT_VERIFY_GRAFTLEAF && !(flag & SCRIPT_VERIFY_TAPROOT)) continue;
