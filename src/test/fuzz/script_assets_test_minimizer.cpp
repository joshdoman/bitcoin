...
const std::map<std::string, script_verify_flag_name> FLAG_NAMES = {
    ...
    {std::string("SINGLETON"), SCRIPT_VERIFY_SINGLETON},
};
...
std::vector<script_verify_flags> AllFlags()
{
    ...
    for (unsigned int i = 0; i < 256; ++i) {
        ...
        if (i & 128) flag |= SCRIPT_VERIFY_SINGLETON;
        ...
        // SCRIPT_VERIFY_SINGLETON requires SCRIPT_VERIFY_TAPROOT
        if (flag & SCRIPT_VERIFY_SINGLETON && !(flag & SCRIPT_VERIFY_TAPROOT)) continue;
