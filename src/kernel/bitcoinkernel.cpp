    ...
    const btck_TransactionOutput** spent_outputs_, size_t spent_outputs_len)
{
    try {
        ...
        return btck_PrecomputedTransactionData::create(std::move(txdata));
    } catch (...) {
        return nullptr;
    }
}

btck_PrecomputedTransactionData* btck_precomputed_transaction_data_copy(const btck_PrecomputedTransactionData* precomputed_data)
{
    return btck_PrecomputedTransactionData::copy(precomputed_data);
}

void btck_precomputed_transaction_data_destroy(btck_PrecomputedTransactionData* precomputed_data)
{
    delete precomputed_data;
}

int btck_script_pubkey_verify(const btck_ScriptPubkey* script_pubkey,
                              const int64_t amount,
                              const btck_Transaction* tx_to,
                              const btck_PrecomputedTransactionData* precomputed_data,
                              ...
                              btck_ScriptVerifyStatus* status)
{
    ...
    const PrecomputedTransactionData& txdata{precomputed_data ? btck_PrecomputedTransactionData::get(precomputed_data) : PrecomputedTransactionData(tx)};
